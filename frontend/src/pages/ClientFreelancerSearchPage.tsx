...
export default function ClientFreelancerSearchPage() {
  ...
  useEffect(() => {
    ...
    getRanking(0, 500)
      .then((data: FreelancerRankingDto[]) => {
  ...
  const skills = useMemo(() => {
    return Array.from(
      new Set(ranking.map((freelancer) => freelancer.skill).filter(Boolean))
    ).sort();
  }, [ranking]);

  const seniorities = useMemo(() => {
    return Array.from(
      new Set(ranking.map((freelancer) => freelancer.seniority).filter(Boolean))
    ).sort();
  }, [ranking]);
  ...
  const recommendedFreelancers = useMemo(() => {
    return ranking
      ...
      .filter(
        (freelancer) => seniority === "ALL" || freelancer.seniority === seniority
      )
      .filter(
        (freelancer) => Number(freelancer.yearsExperience || 0) >= minExperience
      )
      .sort((a, b) => Number(b.finalScore || 0) - Number(a.finalScore || 0))
      .slice(0, 5);
  }, [ranking, skill, seniority, minExperience]);

  const openFreelancerProfile = (freelancer: FreelancerRankingDto) => {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-3xl p-8 mb-8">
          ...
          <h1 className="text-4xl font-bold mt-2">
            Chercher des freelancers recommandés
          </h1>

          <p className="text-blue-100 mt-3 max-w-3xl leading-7">
            Le client choisit ses critères. Le système garde uniquement les
            freelancers disponibles, puis affiche les meilleurs profils selon le
            score final.
          </p>
        </div>
        ...
        {loading && (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 text-slate-300">
            Chargement des freelancers depuis le backend...
          </div>
        )}

        {!loading && error && (
          <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6 text-red-300 mb-8">
            {error}
          </div>
        )}
              ...
              {recommendedFreelancers.map((freelancer, index) => (
                ...
                >
                  ...
                  <div className="mt-4 space-y-2 text-sm">
                    ...
                    <Info
                      label="Seniorité"
                      value={formatLabel(freelancer.seniority)}
                    />

                    <Info
                      label="Expérience"
                      value={`${freelancer.yearsExperience || 0} ans`}
                    />

                    <Info
                      label="Disponibilité"
                      value={formatLabel(freelancer.availabilityStatus)}
                    />

                    <Info
                      label="Tendance"
                      value={formatLabel(freelancer.trendLabel)}
                    />
                  </div>
